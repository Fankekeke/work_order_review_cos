...
Page({
	...
	edit(event) {
		wx.redirectTo({
			url: '/pages/addresss/edit/index?addressId='+event.currentTarget.dataset.addressid+''
		});
	},
	add: function () {
		let that = this;
				...
				if (res.authSetting['scope.address']) {
					wx.chooseAddress({
						success(res) {
							...
							let data = { address, contactPerson: res.userName, contactMethod: res.telNumber, userId: that.data.userInfo.id, defaultAddress: 0, province: res.provinceName, city: res.cityName, area: res.countyName, houseNumber: res.detailInfo, longitude: 116.29607800, latitude: 40.01801800 }
